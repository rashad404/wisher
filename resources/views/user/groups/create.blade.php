...
<div class="max-w-4xl mx-auto py-12">
    <h1 class="text-2xl font-bold text-gray-900 mb-8">{{ isset($group) ? 'Edit' : 'Create' }} Group</h1>

    <form method="POST" action="{{ isset($group) ? route('user.groups.update', $group->id) : route('user.groups.store') }}" class="bg-white p-6 rounded-lg shadow-lg">
        @csrf
        @if(isset($group))
            @method('PUT')
        @endif
        ...
        <div class="text-right">
            <button type="submit" class="inline-flex items-center px-6 py-3 bg-orange-500 text-white text-sm font-semibold rounded-lg shadow hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition duration-200">
